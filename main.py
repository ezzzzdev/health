...
async def register_face(request: Request, image: UploadFile = File(...), user_name: str = Form(...)):
    ...
    try:
        # 얼굴 임베딩 추출
        uploaded_face_encoding = model.get_embedding(face_image)
        
        # static/images 폴더에서 이미 등록된 얼굴들의 인코딩을 로드
        registered_faces = load_faces()  # 이미 등록된 얼굴들의 인코딩 로드 함수
        # 업로드된 얼굴과 등록된 얼굴들 비교
        # 디버깅: registered_faces의 내용을 확인
        print("Registered Faces Loaded:", registered_faces)

        # for문 실행 여부 확인을 위한 디버깅 메시지
        if not registered_faces:
            print("No registered faces found. Check 'registered_faces.json' file.")
        else:
            print("Processing registered faces...")
        # for registered_user_name, registered_data in registered_faces.items():
        #     print("for문은 들어옴")
        #     image_path = f"static/images/{registered_user_name}.jpg"
        #     if not os.path.exists(image_path):  # 파일이 존재하지 않으면 이 사용자는 무시
        #         print("if문 안에 있음")    
        #     # 올바른 방식으로 registered_embedding을 얻음
        #     registered_embedding = np.array(registered_data["embedding"])
        #     print("aaaaaaaaa")
        #     distance = np.linalg.norm(uploaded_face_encoding-registered_embedding)
        ...
        for registered_user_name, registered_encoding in registered_faces.items():
            # registered_encoding을 numpy 배열로 명시적 변환
            registered_embedding = np.array(registered_faces["embedding"])
            # registered_embedding = np.array(registered_faces)
            distance = np.linalg.norm(registered_embedding - uploaded_face_encoding)
            # 파일 존재 여부 확인(얼굴파일 직접 삭제 시 json에는 정보 남아있는 문제때문에)
        ...
        #     if distance < 0.4:  # 임계값 설정, 필요에 따라 조정 가능
        #         print(distance)
        #         return HTMLResponse(content="이미 등록된 사용자입니다.")
    ...
    try:
        ...
        print("밑으로 내려옴")
        new_data = {
            "embedding": uploaded_face_encoding.tolist(),  # numpy 배열을 리스트로 변환
            "image_data": base64.b64encode(image_bytes).decode('utf-8')  # 이미지 데이터를 base64로 인코딩
        }
        registered_faces[user_name] = new_data
        print(new_data)
